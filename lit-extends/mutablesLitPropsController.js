export class MutablesLitPropsController {
    ...
    copyWithin(prop, ...params) {
        ...
        try {
            this.#getRefFromObject(this.host, parts[0])[parts[1]].copyWithin(
                ...params
            );
            this.host.requestUpdate();
        } catch (err) {
    ...
    fill(prop, ...params) {
        ...
        const parts = prop.split(/\.(?=[^.]+$)/);
        try {
            this.#getRefFromObject(this.host, parts[0])[parts[1]].fill(
                ...params
            );
            this.host.requestUpdate();
        } catch (err) {
    ...
    push(prop, ...params) {
        ...
        const parts = prop.split(/\.(?=[^.]+$)/);
        try {
            this.#getRefFromObject(this.host, parts[0])[parts[1]].push(
                ...params
            );
            this.host.requestUpdate();
        } catch (err) {
    ...
    splice(prop, ...params) {
        ...
        const parts = prop.split(/\.(?=[^.]+$)/);
        try {
            this.#getRefFromObject(this.host, parts[0])[parts[1]].splice(
                ...params
            );
            this.host.requestUpdate();
        } catch (err) {
    ...
    unshift(prop, ...params) {
        ...
        const parts = prop.split(/\.(?=[^.]+$)/);
        try {
            this.#getRefFromObject(this.host, parts[0])[parts[1]].unshift(
                ...params
            );
            this.host.requestUpdate();
        } catch (err) {
